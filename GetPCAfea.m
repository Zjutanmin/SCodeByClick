...
MGB = 5;
isRar = 0;
if numel(X) * 8 > MGB*1e9
    isRar = 1;
    iNum = MGB*1e9/8/size(X,2);
    ...
    if nargin < 5
        ...
        if isRar
            X_pca = GetPCATest(X_O, Y_O, PCAmethod, COEFF, xmean, featsize, LocalPCA);    
        end
    else
...
switch PCAMethod
    case 'PCA'
        X_pca = full(X - repmat(xmean, [size(X,1),1])) * COEFF;
    case 'SAE'
...
%         find(~isnan(sum(X.^2, 2)))
        if energy < 1
            ...
            [COEFF, SCORE, LATENT] = princomp(full(X));
